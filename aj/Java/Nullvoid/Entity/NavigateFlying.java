...
public class NavigateFlying extends PathNavigate {
	...
	public NavigateFlying(EntityLiving par1EntityLiving, World par2World) {
	...
	public void onUpdateNavigation() {
		...
		if (!this.noPath()) {
			...
			if (!this.noPath()) {
				...
				if (vec3 != null) {
					this.theEntity.getMoveHelper().setMoveTo(vec3.xCoord,
							vec3.yCoord, vec3.zCoord, this.speed);
				}
			}
		}
	}
	...
	private void pathFollow() {
		...
		for (int j = this.currentPath.getCurrentPathIndex(); j < this.currentPath
				.getCurrentPathLength(); ++j) {
		...
		for (k = this.currentPath.getCurrentPathIndex(); k < i; ++k) {
			if (vec3.squareDistanceTo(this.currentPath.getVectorFromIndex(
					this.theEntity, k)) < (double) f) {
		...
		for (int j1 = i - 1; j1 >= this.currentPath.getCurrentPathIndex(); --j1) {
			if (this.isDirectPathBetweenPoints(vec3,
					this.currentPath.getVectorFromIndex(this.theEntity, j1), k,
					l, i1)) {
	...
	private Vec3 getEntityPosition() {
		return Vec3.createVectorHelper(this.theEntity.posX,
				(double) this.getPathableYPos(), this.theEntity.posZ);
	}
	...
	private void removeSunnyPath() {
		if (!this.worldObj.canBlockSeeTheSky(
			...
			for (int i = 0; i < this.currentPath.getCurrentPathLength(); ++i) {
				...
				if (this.worldObj.canBlockSeeTheSky(pathpoint.xCoord,
						pathpoint.yCoord, pathpoint.zCoord)) {
	...
	private boolean isDirectPathBetweenPoints(Vec3 par1Vec3, Vec3 par2Vec3,
		...
		if (d2 < 1.0E-8D) {
			...
			if (!this.isSafeToStandAt(l, (int) par1Vec3.yCoord, i1, par3, par4,
					par5, par1Vec3, d0, d1)) {
				return false;
			} else {
				par3 -= 2;
				...
				} while (this.isSafeToStandAt(l, (int) par1Vec3.yCoord, i1,
						par3, par4, par5, par1Vec3, d0, d1));
	...
	private boolean isSafeToStandAt(int par1, int par2, int par3, int par4,
		...
		if (!this.isPositionClear(k1, par2, l1, par4, par5, par6, par7Vec3,
				par8, par10)) {
			return false;
		} else {
