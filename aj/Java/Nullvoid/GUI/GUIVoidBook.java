...
public class GUIVoidBook extends GuiScreen {
	private static final Logger logger = LogManager.getLogger();
	private static final ResourceLocation bookGuiTextures = new ResourceLocation("nullvoid", "textures/gui/voidBookGUI.png");
	...
	private GUIVoidBook.NextPageButton buttonNextPage;
	private GUIVoidBook.NextPageButton buttonPreviousPage;
	...
	public void initGui() {
		...
		byte b0 = 2;
		this.buttonList
				.add(this.buttonNextPage = new GUIVoidBook.NextPageButton(1,
						i + 120, b0 + 154, true));
		this.buttonList
				.add(this.buttonPreviousPage = new GUIVoidBook.NextPageButton(
						2, i + 38, b0 + 154, false));
		this.updateButtons();
	}
		...
		if (this.bookIsUnsigned && this.field_146481_r) {
			if (this.bookPages != null) {
				...
				while (this.bookPages.tagCount() > 1) {
					s = this.bookPages
							.getStringTagAt(this.bookPages.tagCount() - 1);
				...
				if (this.bookObj.hasTagCompound()) {
					NBTTagCompound nbttagcompound = this.bookObj
							.getTagCompound();
				...
				if (p_146462_1_) {
					s = "MC|BSign";
					this.bookObj.setTagInfo("author", new NBTTagString(
							this.editingPlayer.getCommandSenderName()));
					this.bookObj.setTagInfo("title", new NBTTagString(
							this.bookTitle.trim()));
				...
				try {
					(new PacketBuffer(bytebuf))
							.writeItemStackToBuffer(this.bookObj);
					this.mc.getNetHandler().addToSendQueue(
							new C17PacketCustomPayload(s, bytebuf));
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		if (this.field_146480_s) {
			...
			this.fontRendererObj.drawString(s,
					k - l + this.bookImageWidth - 44, b0 + 16, 0);
			this.fontRendererObj.drawSplitString(s1, k + 36, b0 + 16 + 16, 116,
					0);
		}
